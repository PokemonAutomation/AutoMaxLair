...
def print_matchup_summary(attacker: Pokemon, defender: Pokemon,
    ...
    output = f'Matchup between {attacker.name_id} and {defender.name_id}: %0.2f' % evaluate_matchup(
        attacker, defender, teammates)
    print(output)
    for i in range(len(attacker.moves)):
        move_list = attacker.max_moves if attacker.dynamax else attacker.moves
        output = 'Score for ' + move_list[i].name_id + ' (Effective BP %i, accuracy %0.2f, type multiplier %0.1f): ' % (
            move_list[i].power, move_list[i].accuracy, type_damage_multiplier(move_list[i].type_id, defender.type_ids))
