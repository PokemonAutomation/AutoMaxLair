...
def select_pokemon(inst) -> str:
    ...
    for i in range(inst.num_caught):
        if inst.check_shiny():
            ...
            if inst.check_sufficient_ore(
                inst.calculate_ore_cost(inst.consecutive_resets + 1) +
                inst.calculate_ore_cost(inst.consecutive_resets + 2)
            ):
                reset_game = True
                break
            else:
            ...
            inst.push_buttons((b'^',3))
                ...
                inst.check_sufficient_ore(inst.calculate_ore_cost(consecutive_resets + 1))
            ):
            reset_game = True
