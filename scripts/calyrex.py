"""Script for soft resetting to get 0 Atk IVs calyrex

Instructions:
1. Press the '+' button until the IVs of Pokemon in your boxes are shown.
2. Have your party missing 2 slots.
3. Save directly in front of calyrex.
4. Connect the microcontroller, serial connection, and capture card, then start
   the script.
"""
...
# Set the log name
LOG_NAME = f"calyrex_{datetime.now().strftime('%Y-%m-%d %H-%M-%S')}"


class CalyrexController(automaxlair.switch_controller.SwitchController):
    """Switch controller specific to resetting for 0 Atk calyrex."""
...
def loop(ctrlr) -> None:
    ...
    for __ in range(45):
        ctrlr.push_button(b'a', 1)

    ctrlr.log('Splitting calyrex and spectrier.')
    ctrlr.push_buttons(
        (b'x', 1.5), (b'>', 0.5), (b'>', 0.5),
        (b'a', 2), (b'<', 0.5), (b'^', 0.5),
        (b'a', 2), (b'a', 2), (b'^', 2),
    ...
    ctrlr.log('Checkin stats.')
    ctrlr.push_buttons(
        (b'x', 2), (b'<', 0.5), (b'a', 2),
        (b'r', 3), (b'<', 1), (b'^', 1),
        (b'^', 1 + VIDEO_EXTRA_DELAY))
    ...
    ctrlr.push_button(b'^', 1 + VIDEO_EXTRA_DELAY)
    # Check the Atk IV and quit if it's 0 ("No good")
    IV_text = ctrlr.read_text(ctrlr.get_frame(), ctrlr.IV_atk_rect)
    if 'pastop' in IV_text.lower().replace(' ', ''):
        ctrlr.log('********** Found 0 Atk target! **********')
...
def main(log_name: str) -> None:
    ...
    controller = CalyrexController(config, log_name, actions)
